...
public class connector{
    ...
    /**
     * Initializes a new {@link connector} instance by acting as a server, blocking
     ...
     *
     */
    public connector(ServerSocket s,IAuthenticate a) throws IOException{
        io(s.accept());
        rBuff=new byte[99];
    ...
    /**
     * Initializes a new {@link connector} instance by acting as a server, and blocks while waiting
     * for a new client connection. A new instance should be created for each new client connection.
     * @param s {@link ServerSocket} to accept connections on using kdb+ IPC protocol.
     * @throws IOException an I/O error occurs.
     */
    public connector(ServerSocket s) throws IOException{
        this(s,null);
    }
    /**
     * Initializes a new {@link connector} instance and connects to KDB+ over TCP.
     ...
     */
    public connector(String host,int port,String usernamepassword) throws KException,IOException{
        this(host,port,usernamepassword,false);
    }

    /**
     * Initializes a new {@link connector} instance and connects to KDB+ over TCP with optional TLS support for encryption.
     * @param host Host of remote q process
     * @param port Port of remote q process
     * @param usernamepassword Username and password as "username:password" for remote authorization
     ...
     * @throws IOException if an I/O error occurs.
     */
    public connector(String host,int port,String usernamepassword,boolean useTLS) throws KException,IOException{
    ...
    /**
     * Initializes a new {@link connector} instance and connects to KDB+ over TCP, using {@code user.name} system property for username and password criteria.
     ...
     * @throws IOException if an I/O error occurs.
     */
    public connector(String host,int port) throws KException,IOException{
        this(host,port,System.getProperty("user.name"));
    }
    /** Initializes a new {@link connector} instance for the purposes of serialization only, no connection is instantiated
...
//object.getClass().isArray()   t(int[]) is .5 isarray is .1 lookup .05
     ...
     * @throws UnsupportedEncodingException  If the named charset is not supported
     */
    public static int nx(Object x) throws UnsupportedEncodingException{
        int type=t(x);
        if(type==99)
    ...
    public interface MsgHandler{
         ...
         */
        default void processMsg(connector c,byte msgType,Object msg)throws IOException{
            switch(msgType){
     ...
     */
    public static final Object[] NULL={null, Boolean.FALSE,new UUID(0,0),null, (byte) 0, Short.MIN_VALUE, ni, nj, (float) nf, nf, ' ',"",
